...
import { ArrowLeft, Loader2, ChevronDown, Brain, Zap, MessageSquare, Lightbulb, Heart, Scale, HelpCircle, Clock } from "lucide-react";
...
const GenerateAudition = () => {
  ...
  const scaffoldQuery = useQuery({
    ...
    staleTime: 15000,  // Keep data fresh for 15s across navigations
    placeholderData: (previousData) => previousData,  // Preserve cached data on remount
    retry: 1,
  ...
  useEffect(() => {
    ...
    if (scaffold?.status === 'GENERATING') {
      ...
      setClientElapsedSeconds(prev => Math.max(prev, serverElapsedSeconds));
              ...
              >
                ...
                <CollapsibleContent className="mt-4">
                  ...
                  <li className="flex items-start gap-2">
                    ...
                    <span>Generating 15 custom questions across 6 performance dimensions</span>
                  </li>
                      <li className="flex items-start gap-2">
                        <span className="text-primary mt-0.5">•</span>
                  ...
                  <li className="flex items-start gap-2">
                    <span className="text-primary mt-0.5">•</span>
                    <span>Selecting 10 questions: 5 from top dimensions, 5 from secondary</span>
                  </li>
                      <li className="flex items-start gap-2">
                        <span className="text-primary mt-0.5">•</span>
        ...
        <div className="flex flex-col items-center gap-4 rounded-lg border border-destructive/40 bg-destructive/10 p-6 text-center">
          <div className="flex flex-col items-center gap-2">
            <p className="text-base font-medium text-destructive">
              {scaffold.message || 'Question Bank Generation Failed'}
            </p>
            {scaffold.error_code ? (
              <Badge variant="outline" className="text-[10px] uppercase tracking-wide text-destructive border-destructive/40">
                {scaffold.error_code}
              </Badge>
            ) : null}
          </div>
          ...
          <Button onClick={async () => {
            ...
            try {
              ...
              if (error) {
                toast({
                  title: "Retry failed",
                  description: error.message || "Could not retry generation.",
                  variant: "destructive",
                });
                return;
              }
