...
5. A structured "Candidate-Facing JD" in VettedAI format.

Always call the extract_role_data function with properly structured arguments. Do not include markdown or code fences.

JSON Schema: { "definition_data": {...}, "context_flags": {...}, "clarifier_inputs": {...}, "clarifier_questions": [...], "candidate_facing_jd": {...} }
...
For the candidate_facing_jd field, write in a direct, human, high-context tone like a message to a smart operator in your network. Always use second-person ("You've... You know how to..."):
- company_intro: Write a short, high-level intro (2-3 sentences). Focus on why this company, why now. Be direct and human.
- engagement_description: This is the hook (2-4 sentences). Focus on the strategic challenge, real-world differentiation, and impact of the role. Avoid generic fluff. Write in second-person ("You'll be...").
- deliverables: Extract key responsibilities as an array of clear, specific, outcome-focused bullet points (4-6 items). Start each with an action verb.
- skills_and_requirements: Write as an array of "You might be a great fit if..." statements (4-6 items). Focus on mindset, instincts, and working style over simple credential checklists. Each should be conversational and specific.
- nice_to_have: Extract all "bonus points" or "nice to haves" (2-4 items). If none are found, return an empty array.
- location: Extract the location. Be honest about flexibility and remote work (e.g., "Remote (US)", "New York, NY (Hybrid)", "San Francisco (In-office 5 days/week)").
- compensation: Extract all compensation details (salary range, equity, bonus, etc.). Be honest about ambiguity (e.g., "Competitive salary based on experience" or "$120k-$160k + equity"). If not mentioned, use "Competitive compensation package".`;

/**
 * Generate a deterministic hash from job description text
 * Returns first 8 characters of hash for readability
 */
function hashJobDescription(jdText: string): string {
  // Normalize text: lowercase, remove extra whitespace
  const normalized = jdText.toLowerCase().trim().replace(/\s+/g, ' ');
  ...
  // Convert to hex and take first 8 chars for readability
...
type RoleDefinitionResult = {
  ...
  candidate_facing_jd: CandidateFacingJD;
};
...
function generateBankId(
  ...
  // Start with JD hash for uniqueness
  let bankId = `${jdHash}-${normalizedFamily}-${contextFlags.seniority}`;
  ...
  console.log(`Generated bank_id: ${bankId}`);
  return bankId;
}
...
function computeWeightedDimensions(
  ...
  // Generate Bank ID with JD hash
  const bank_id = generateBankId(contextFlags, jdHash);
...
async function callAiGateway(jdText: string): Promise<RoleDefinitionResult> {
          ...
          function: {
            ...
            parameters: {
              ...
              properties: {
                ...
                candidate_facing_jd: {
                  type: "object",
                  description: "Structured candidate-facing job description in VettedAI format",
                  properties: {
                    company_intro: {
                      type: "string",
                      description: "2-3 sentence company intro focusing on 'why this company, why now'"
                    },
                    engagement_description: {
                      type: "string", 
                      description: "2-4 sentence hook describing the strategic challenge and impact. Written in second-person."
                    },
                    deliverables: {
                      type: "array",
                      items: { type: "string" },
                      description: "4-6 clear, specific, outcome-focused responsibilities starting with action verbs"
                    },
                    skills_and_requirements: {
                      type: "array",
                      items: { type: "string" },
                      description: "4-6 'You might be a great fit if...' statements focusing on mindset and working style"
                    },
                    nice_to_have: {
                      type: "array",
                      items: { type: "string" },
                      description: "2-4 bonus qualifications. Empty array if none found."
                    },
                    location: {
                      type: "string",
                      description: "Location with flexibility details (e.g., 'Remote (US)', 'NYC (Hybrid)')"
                    },
                    compensation: {
                      type: "string",
                      description: "Compensation details or 'Competitive compensation package' if not specified"
                    }
                  },
                  required: [
                    "company_intro",
              ...
              required: ["definition_data", "context_flags", "clarifier_inputs", "clarifier_questions", "candidate_facing_jd"],
              additionalProperties: false,
            },
          },
        },
  ...
  // Generate hash from original JD text
  const jdHash = hashJobDescription(jdText);
  console.log(`JD hash: ${jdHash}`);
