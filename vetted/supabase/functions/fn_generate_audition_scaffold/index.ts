...
function formatBankFailureCopy(reason?: string | null): FailureCopy {
  if (!reason) {
    return {
      message: 'Generation failed before we could cache your question bank.',
  ...
  if (normalized.includes('RATE_LIMIT') || normalized.includes('SERVICE_OVERLOADED')) {
    return {
      message: 'Lovable temporarily rate limited the question generator.',
      error: 'Lovable returned repeated rate limits while generating questions. Wait ~60 seconds, then click "Retry Generation".',
      errorCode: 'RATE_LIMIT'
    };
  }

  if (normalized.includes('SERVICE_UNAVAILABLE') || normalized.includes('SERVER_ERROR_')) {
    return {
      message: 'Lovable AI gateway was unavailable during generation.',
      error: `${reason}. Please try again once the service recovers.`,
      errorCode: 'UPSTREAM_UNAVAILABLE'
    };
  }

  return {
    message: 'Question bank generation failed.',
    error: reason,
...
/**
 * Select 5 Core questions (highest weighted dimensions)
 */
function selectCoreQuestions(
  ...
  // Take 5, ensuring diversity across archetypes
  ...
  for (const question of shuffled) {
    ...
    if (!usedArchetypes.has(question.archetype_id) || selected.length > 3) {
...
/**
 * Select 5 Variable questions (remaining dimensions)
 */
function selectVariableQuestions(
  ...
  // Take 5, ensuring dimension diversity
  const selected: QuestionObject[] = [];
  ...
  for (const question of shuffled) {
    ...
    // Limit to max 2 questions per dimension
    if (dimCount < 2) {
      selected.push(question);
...
/**
 * Main selection logic: 5 Core + 5 Variable
 */
function selectQuestions(
  ...
  console.log(`🎲 Selecting 10 questions from ${bank.questions.length} available`);
  
  if (bank.questions.length < 10) {
    throw new Error(`Insufficient questions in bank: ${bank.questions.length} (need 10)`);
  }
  
  const seed = projectId; // Use project_id for deterministic selection
  
  // Step 1: Select 5 Core questions
  ...
  // Step 2: Select 5 Variable questions
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { project_id, retry } = await req.json();
    ...
    if (bank && bank.status === 'GENERATING') {
      const elapsedMs = Date.now() - new Date(bank.updated_at).getTime();
      const elapsedMinutes = elapsedMs / 60000;
      
      console.log(`🟡 PENDING: Bank is generating (${elapsedMinutes.toFixed(1)} min elapsed)`);
      
      // Always return 202 GENERATING - no automatic resets
      return new Response(JSON.stringify({
        status: 'GENERATING',
        bank_id: bankId,
        ...
        estimated_remaining_minutes: Math.max(0, Math.ceil(2 - elapsedMinutes)),
    ...
    // ========================================================================
    // PATH 3: FAILED (bank exists but generation failed)
    // ========================================================================
    if (bank && bank.status === 'FAILED') {
      const elapsedMs = Date.now() - new Date(bank.updated_at).getTime();
      const elapsedSeconds = elapsedMs / 1000;

      console.log(`🔴 FAILED: Bank generation failed (${elapsedSeconds.toFixed(0)}s ago)`);

      // Handle manual retry from UI
      if (retry === true && elapsedSeconds >= 30) {
        console.log('♻️ RETRY: Resetting FAILED bank to GENERATING');
        
        // Reset bank status to GENERATING
        ...
        if (resetError) {
          throw new Error(`Failed to reset bank: ${resetError.message}`);
        }
        
        // Re-invoke question factory
        await invokeQuestionFactory(supabase, roleDefinition, bankId);
        
        return new Response(JSON.stringify({
          status: 'GENERATING',
          bank_id: bankId,
          message: 'Retrying question bank generation. This typically takes 2-3 minutes.',
          elapsed_minutes: 0,
      ...
      const failureCopy = formatBankFailureCopy(bank.failure_reason);

      // Return FAILED status - let UI handle manual retry via button
      return new Response(JSON.stringify({
        status: 'FAILED',
        bank_id: bankId,
        message: failureCopy.message,
        error: failureCopy.error,
        failure_reason: bank.failure_reason,
        error_code: failureCopy.errorCode,
        retry_after_seconds: Math.max(0, Math.ceil(30 - elapsedSeconds))
      }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    ...
    // ========================================================================
    // PATH 4: COLD START (bank does not exist)
    // ========================================================================
