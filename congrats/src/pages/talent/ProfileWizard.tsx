...
export default function ProfileWizard() {
  ...
  const handleConfirmFinish = async () => {
    ...
    try {
      ...
      // Check if user has a pending job application
      ...
      if (pendingJob) {
        try {
          ...
          // Redirect back to the job they were applying to
          navigate(`/jobs/${jobData.orgSlug}/${jobData.jobSlug}`, {
  ...
  // Redirect if profile is already complete (skip the smartProgress check)
  useEffect(() => {
    if (!wizardLoading && !isLoading && profile?.onboarding_completed) {
      toast.success("Your profile is already complete!");
      navigate("/jobs");
    }
  }, [wizardLoading, isLoading, profile?.onboarding_completed, navigate]);
  ...
  // Fallback: If no incomplete steps and wizard not loading, default to step 1
  const activeStep = incompleteSteps.length > 0 ? currentWizardStep : 1;
    ...
    <div className="container max-w-3xl mx-auto py-8 px-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Step {Math.max(currentStepIndex + 1, 1)} of {Math.max(totalIncomplete, 1)}: {stepTitles[activeStep as WizardStep] || stepTitles[1]}
          </CardDescription>
          <Progress value={smartProgress} className="mt-4" />
        </CardHeader>
        <CardContent className="pt-6">
          {(shouldRenderStep(1) || incompleteSteps.length === 0) && activeStep === 1 && (
            <NameStep
              defaultValues={{ first_name: formData.first_name, last_name: formData.last_name }}
              onNext={handleNameNext}
            />
          )}
          {shouldRenderStep(2) && activeStep === 2 && (
            <LinkedInStep
              defaultValues={{ linkedin_url: formData.linkedin_url }}
              onNext={handleLinkedInNext}
            />
          )}
          {shouldRenderStep(3) && activeStep === 3 && (
            <CategoriesStep
          ...
          {shouldRenderStep(4) && activeStep === 4 && (
            <ExperienceLevelStep
          ...
          {shouldRenderStep(5) && activeStep === 5 && (
            <CurrentRoleStep
          ...
          {shouldRenderStep(6) && activeStep === 6 && (
            <WorkPreferencesStep
              defaultValues={{
          ...
          {shouldRenderStep(7) && activeStep === 7 && (
            <SalaryStep
          ...
          {shouldRenderStep(8) && activeStep === 8 && (
            <ResumeStep
              onBack={goToPrevious}
              onFinish={handleResumeNext}
            />
          )}
          {shouldRenderStep(9) && activeStep === 9 && (
            <ConfirmStep
              onBack={goToPrevious}
